...
$fs = get_file_storage();
$files = $fs->get_area_files($context->id, 'mod_signature', 'content', 0, 'sortorder DESC, id DESC', false); // TODO: this is not very efficient!!
...
$path = '/'.$context->id.'/mod_signature/content/0/'.$file->get_filename();
$fullurl = moodle_url::make_file_url('/pluginfile.php', $path );
$pdfurl = $CFG->wwwroot.'/pluginfile.php'.$path;
$iframe = '<iframe width="100%" height="800px" src="'.$fullurl.'"></iframe>';

$content = $moduleinstance->signature_content;


if(isset( $_POST['hiddenSigDataa'] ) && !empty(  $_POST['hiddenSigDataa']  )  ){
...
<div id="content">
	...
	<?php if(isset( $_POST['hiddenSigDataa'] ) ){
		...
		echo $template;
		// $path = '/'.$cm->id.'/mod_signature/content/'.$signatureid.'/'.$file->get_filename();
		// $fullurl = moodle_url::make_file_url('/pluginfile.php', $path );
		?>
